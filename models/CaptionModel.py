...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, core_args, **kwargs):
        ...
        state_table = [
            list(torch.unbind(_)) for _ in torch.stack(init_state).chunk(group_size, 2)
        ]
        ...
        core_args = [
            _.chunk(group_size)
            if (_ is not None and type(_) is not list)
            else [_] * group_size
            for _ in core_args
        ]
        core_args = [
            [core_args[i][j] for i in range(len(core_args))] for j in range(group_size)
        ]
            ...
            for divm in range(group_size):
                if t >= divm and t <= self.seq_length + divm - 1:
                    ...
                    logprobs_table[divm], state_table[divm] = self.get_logprobs_state(
                        it.cuda(), state_table[divm], core_args[divm]
                    )
