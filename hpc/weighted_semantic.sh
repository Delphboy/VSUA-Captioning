...
python3 train.py --batch_size 32 \
                ...
                --id weighted-semantic-seed=${seed}-split=${split}-learning_rate=${learning_rate}-learning_rate_decay_rate=${learning_rate_decay_rate}-dropout=${dropout}-rnn_size=${rnn_size}-input_encoding_size=${input_encoding_size}-att_hid_size=${att_hid_size}-num_layer=${num_layer} \
