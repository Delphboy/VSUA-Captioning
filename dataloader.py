...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.sg_vocab = np.load(opt.sg_vocab_path, allow_pickle=True)[()]
        ...
        for split in self.split_ix.keys():
            self.logger.info(
                "assigned %d images to split %s" % (len(self.split_ix[split]), split)
            )
        ...
        for split in self.iterators.keys():
            self._prefetch_process[split] = BlobFetcher(
                split, self, split == "train", self.opt.loader_num_workers, self.opt
            )
    ...
    def get_captions(self, ix, seq_per_img):
        ...
        if ncap < seq_per_img:
            ...
            seq = self.h5_label_file["labels"][
                ixl : ixl + seq_per_img, : self.seq_length
            ]
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        att_batch = []
        # np.ndarray((batch_size * seq_per_img, 14, 14, self.opt.att_feat_size),
        #           dtype='float32')
        sg_batch = []
        ...
        for i in range(batch_size):
            # fetch image
            tmp_fc, tmp_att, tmp_sg, ix, tmp_wrapped = self._prefetch_process[
                split
            ].get()
            ...
            label_batch[
                i * seq_per_img : (i + 1) * seq_per_img, 1 : self.seq_length + 1
        ...
        data["fc_feats"] = np.stack(
            reduce(lambda x, y: x + y, [[_] * 1 for _ in fc_batch])
        )
        # merge att_feats
    ...
    def batch_sg(self, sg_batch, max_att_len):
        ...
        sg_data["obj_labels"] = np.zeros(
            [len(obj_batch), max_att_len, self.opt.num_obj_label_use], dtype="int"
        )
        for i in range(len(obj_batch)):
            sg_data["obj_labels"][i, : obj_batch[i].shape[0]] = obj_batch[i]

        # attr labels, shape: (B, No, 3)
        sg_data["attr_labels"] = np.zeros(
            [len(attr_batch), max_att_len, self.opt.num_attr_label_use], dtype="int"
        )
        ...
        if self.geometry_relation:
            ...
            sg_data["rela_feats"] = np.zeros(
                [len(rela_batch), max_rela_len], dtype="int"
            )
        # rela_masks, because no all items in rela_edges and rela_feats are meaningful
        sg_data["rela_masks"] = np.zeros(
            sg_data["rela_edges"].shape[:2], dtype="float32"
        )

        for i in range(len(rela_batch)):
            sg_data["rela_edges"][i, : rela_batch[i]["edges"].shape[0]] = rela_batch[i][
                "edges"
            ]
            sg_data["rela_feats"][i, : rela_batch[i]["edges"].shape[0]] = rela_batch[i][
                "feats"
            ]
    ...
    def __getitem__(self, index):
        ...
        if self.use_att:
            att_feat = np.load(os.path.join(self.input_att_dir, image_id + ".npz"))[
                "feat"
            ]
            # Reshape to K x C
        ...
        return (fc_feat, att_feat, sg_data, ix)

    def get_graph_data(self, index):
        image_id = str(self.info["images"][index]["id"])
        sg_use = np.load(
            self.sg_data_dir + image_id + ".npy", allow_pickle=True, encoding="latin1"
        )[()]
        if self.geometry_relation:
            geometry_path = os.path.join(self.sg_geometry_dir, image_id + ".npy")
            rela = np.load(geometry_path, allow_pickle=True)[
                ()
            ]  # dict contains keys of edges and feats
        else:
            # if the relation of an image is empty,
            # then fill in it with <0, 0, 'near'> to avoid problems
            if sg_use["rela_matrix"].shape[0] == 0:
                sg_use["rela_matrix"] = np.array(
                    [0, 0, self.sg_vocab["w2i"]["near"]],
                    dtype=sg_use["rela_matrix"].dtype,
                )
        ...
        attr = sg_use["obj_attr"][
            :, 4 : 4 + self.opt.num_attr_label_use
        ]  # shape (No, ?)
...
class BlobFetcher:
    ...
    def __init__(self, split, dataloader, if_shuffle=False, num_workers=4, opt=None):
        """
        db is a list of tuples: imcrop_name, caption, bbox_feat of gt box, imname
        """
        self.opt = opt
        self.split = split
