...
from misc.rewards_graph import get_self_critical_reward, init_scorer
...
from utils.logger import MyTensorboard, define_logger
...
def train(opt):
    ...
    # decoder = nn.DataParallel(decoder) # TODO: Fix multi-GPU support
    crit = utils.LanguageModelCriterion().cuda()
    # crit = nn.DataParallel(crit) # TODO: Fix multi-GPU support
    rl_crit = utils.RewardCriterion().cuda()
    # rl_crit = nn.DataParallel(rl_crit) # TODO: Fix multi-GPU support
    ...
    while True:
        ...
        if not sc_flag:
            ...
            gen_result, sample_logprobs, core_args = decoder(
                ...
                att_feats,
                att_masks,
                opt={"sample_max": 0, "return_core_args": True},
                mode="sample",
            )
        ...
        if (
            opt.save_checkpoint_every > 0 and iteration % opt.save_checkpoint_every == 0
        ) or (opt.save_checkpoint_every == -1 and epoch_done):
            # eval model
            eval_kwargs = {
                "split": "val",
